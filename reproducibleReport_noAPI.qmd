...
mosqDF <- read.csv("suffolkVAMosqData.csv")
...
mosqDFcomb <- mosqDF %>% group_by(sample_start_date) %>% summarize(totalMos = sum(sample_value))
...
daymet <- read.csv("Daymet_Suffolk.csv")

daymet$temp_avg <- (daymet$tmax..deg.c. + daymet$tmin..deg.c.)/2

mosqMod1 <- lm(totalMos ~ time + totalMos_lag1 + sinSeason + cosSeason,
              data = mosqMonth)
